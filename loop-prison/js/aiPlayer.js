...
window.game.aiPlayer = {
  ...
  update(dt) {
    ...
    // ---- 边缘预检测 ----
    if (this.isGrounded && this._moveDistanceRemaining !== 0 && !this._hasJumpedForCommand) {
      var plat = this._edgeDetect();
      if (plat) {
        var targetX = this.x + this._moveDistanceRemaining;
        var beyondEdge = (targetX < plat.x && this._moveDistanceRemaining < 0) ||
                         (targetX + this.w > plat.x + plat.w && this._moveDistanceRemaining > 0);
        if (beyondEdge) {
          var gap = Math.min(Math.abs(targetX - (this._moveDistanceRemaining > 0 ? plat.x + plat.w : plat.x)), 200);
          if (gap > 0 && gap < this.w * 3 && !this._hasJumpedForCommand) {
            this._hasJumpedForCommand = true;
            var jv = this.jumpVelocity !== null ? this.jumpVelocity : (window.game.JUMP_VELOCITY || -380);
            this.vy = jv;
            this.isGrounded = false;
            this._jumpCooldown = 0.3;
            console.log('[AI] 边缘跳跃: gap=' + Math.round(gap));
          }
        }
      }
    }
...
---- 水平移动 + 固体碰撞（距离制） ----
    ...
    if (shouldJump && this.isGrounded && this._jumpCooldown <= 0 && !this._hasJumpedForCommand) {
      // 致命障碍检测：跳跃前预测落点
    if (this.isGrounded && this.vy <= 0 && !this._hasJumpedForCommand) {
      var predY = this.y + (-380) * 0.3 + 0.5 * 800 * 0.09;
      var predRect = { x: this.x, y: Math.min(predY, 560), w: this.w, h: this.h };
      if (map.checkDeadlyCollision && map.checkDeadlyCollision(predRect)) {
        console.log('[AI] 跳跃落点有致命障碍，取消跳跃');
        this._hasJumpedForCommand = true;
        this._moveDistanceRemaining = 0;
        return;
      }
